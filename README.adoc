...
Zur Vorbereitung ist zu beachten:

- "application.properties" muss für den Einsatz mit Docker auf "spring.profiles.active=prod" gesetzt werden.

- Im jeweiligen Profil "application-dev.properties" oder "application-prod.properties" sollte "spring.jpa.hibernate.ddl-auto=xxx" beachtet werden. Optimale Optionen sind "update" für den Produktiveinsatz und "create-drop" zum Testen.

== Starten von Tests

Wenn application.properties auf production eingestellt ist, werden die Tests mit Travis ausgeführt.

Wenn application.properties auf development eingestellt ist, können die Tests mit einer IDE gestartet werden.
 
== Voreingestellte User

Es gibt folgende schon existierende Benutzerkonten:

- Username: "admin", Passwort: "rootroot". Dieser hat Adminrechte.

- Username: "mandypandy", Passwort: "abcdabcd".

- Username: "pandycandy", Passwort: "abcdabcd".

Weitere Benutzer können jederzeit neu registriert werden.
